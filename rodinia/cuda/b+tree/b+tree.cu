...
__global__ void findRangeK(long height, knode *knodesD, long knodes_elem,
...
void kernel_gpu_cuda_wrapper(record *records, long records_mem, knode *knodes,
  ...
  findK<<<numBlocks, threadsPerBlock>>>(maxheight, knodesD, knodes_elem,
                                        recordsD, currKnodeD, offsetD, keysD,
                                        ansD);
  cudaDeviceSynchronize();
...
int main(int argc, char **argv) {
  ...
  int cur_arg;
  // int arch_arg = 0;  // KERMA: unused
  // int cores_arg = 1; // KERMA: unused
  ...
  // For Debug
  // char *sPointer = commandBuffer; // KERMA: unused
